# Copyright 1999-2025 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

EAPI=8

# ebuild generated by hackport 0.9.0.0

CABAL_FEATURES="lib profile haddock hoogle hscolour test-suite"
inherit haskell-cabal

DESCRIPTION="An optimising compiler for a functional, array-oriented language"
HOMEPAGE="https://futhark-lang.org"

LICENSE="ISC"
SLOT="0/${PV}"
KEYWORDS="~amd64"

RDEPEND=">=dev-haskell/aeson-2.0.0.0:=[profile?]
	>=dev-haskell/ansi-terminal-0.6.3.1:=[profile?]
	>=dev-haskell/async-2.0:=[profile?]
	dev-haskell/base16-bytestring:=[profile?]
	>=dev-haskell/blaze-html-0.9.0.1:=[profile?]
	>=dev-haskell/bmp-1.2.6.3:=[profile?]
	>=dev-haskell/bytestring-to-vector-0.3.0.1:=[profile?]
	>=dev-haskell/cmark-gfm-0.2.1:=[profile?]
	dev-haskell/co-log-core:=[profile?]
	dev-haskell/cryptohash-md5:=[profile?]
	>=dev-haskell/diff-0.4.1:=[profile?]
	>=dev-haskell/directory-tree-0.12.1:=[profile?]
	>=dev-haskell/dlist-0.6.0.1:=[profile?]
	dev-haskell/fgl:=[profile?]
	dev-haskell/fgl-visualize:=[profile?]
	>=dev-haskell/file-embed-0.0.14.0:=[profile?]
	>=dev-haskell/free-5.1.10:=[profile?]
	>=dev-haskell/futhark-data-1.1.1.0:=[profile?]
	>=dev-haskell/futhark-manifest-1.5.0.0:=[profile?]
	>=dev-haskell/futhark-server-1.2.3.0:=[profile?]
	>=dev-haskell/githash-0.1.6.1:=[profile?]
	>=dev-haskell/half-0.3:=[profile?]
	dev-haskell/haskeline:=[profile?]
	>=dev-haskell/language-c-quote-0.12:=[profile?]
	dev-haskell/lens:=[profile?]
	>=dev-haskell/lsp-2.2.0.0:=[profile?]
	>=dev-haskell/lsp-types-2.0.1.0:=[profile?]
	>=dev-haskell/mainland-pretty-0.7.1:=[profile?]
	>=dev-haskell/megaparsec-9.0.0:=[profile?]
	dev-haskell/mwc-random:=[profile?]
	>=dev-haskell/neat-interpolation-0.3:=[profile?]
	>=dev-haskell/parallel-3.2.1.0:=[profile?]
	>=dev-haskell/prettyprinter-1.7:=[profile?]
	>=dev-haskell/prettyprinter-ansi-terminal-1.1:=[profile?]
	>=dev-haskell/process-extras-0.7.2:=[profile?]
	>=dev-haskell/random-1.2.0:=[profile?]
	>=dev-haskell/regex-tdfa-1.2:=[profile?]
	>=dev-haskell/srcloc-0.4:=[profile?]
	dev-haskell/statistics:=[profile?]
	dev-haskell/temporary:=[profile?]
	>=dev-haskell/terminal-size-0.3:=[profile?]
	>=dev-haskell/text-1.2.2.2:=[profile?]
	>=dev-haskell/vector-0.12:=[profile?]
	>=dev-haskell/versions-6.0.0:=[profile?]
	>=dev-haskell/zlib-0.7.0.0:=[profile?]
	>=dev-lang/ghc-9.2.4:=
"
DEPEND="${RDEPEND}
	dev-haskell/alex
	>=dev-haskell/cabal-3.6.3.0
	dev-haskell/happy
	test? ( >=dev-haskell/quickcheck-2.8
		dev-haskell/tasty
		dev-haskell/tasty-hunit
		dev-haskell/tasty-quickcheck )
"
