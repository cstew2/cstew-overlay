...
SRC_URI="https://github.com/NationalSecurityAgency/${PN}/archive/Ghidra_${PV}_build.tar.gz
	...
	https://www.eclipse.org/downloads/download.php?file=/tools/cdt/releases/8.6/cdt-8.6.0.zip
	mirror://sourceforge/project/pydev/pydev/PyDev%206.3.1/PyDev%206.3.1.zip -> PyDev-6.3.1.zip
	https://dev.pentoo.ch/~blshkv/distfiles/${PN}-dependencies-20211222.tar.gz"
...
src_unpack() {
	...
	cd "${S}"
	mv ../dependencies . || die "(11) mv failed"
}

src_prepare() {
	mkdir -p ".gradle/init.d"                       || die "(12) mkdir failed"
	cp "${FILESDIR}"/repos.gradle .gradle/init.d    || die "(13) cp failed"
	sed -i "s|S_DIR|${S}|g" .gradle/init.d/repos.gradle || die "(14) sed failed"
	#remove build date so we can unpack dist.zip later
	sed -i "s|_\${rootProject.BUILD_DATE_SHORT}||g" gradle/root/distribution.gradle || die "(15) sed failed"
