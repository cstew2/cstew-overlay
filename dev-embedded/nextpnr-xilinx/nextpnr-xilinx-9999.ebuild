...
PATCHES=( "${FILESDIR}/include_set.patch" )
...
src_compile() {
	...
	"${WORKDIR}"/"${P}_build"/bba/bbasm --le -v --files "${S}/xilinx/xc7a100t.bba" "${S}/xilinx/xc7a100t.bin" || die
}

src_install() {
	cmake_src_install
	dobin "${WORKDIR}/${P}_build"/bba/bbasm
